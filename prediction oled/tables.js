...
function createPredictedPropertiesTable(material, emitterName = "Unknown") {
    const tableStyle = 'border: 1px solid black; border-collapse: collapse; width: 40%;';
    const thStyle = 'border: 1px solid black; padding: 8px; background-color: #f2f2f2; text-align: center;';
    const tdStyle = 'border: 1px solid black; padding: 8px; text-align: center;';

    const predPLQY = material["plqy.1"];
    const predEQE = material["EQE.1"];

    let formattedPredPLQY = typeof predPLQY === "number" ? predPLQY.toPrecision(3) : "N/A";
    let formattedPredEQE  = typeof predEQE  === "number" ? predEQE.toPrecision(3)  : "N/A";

    // 如果 material.emitter 沒有，就用第二個參數
    let emitterLabel = material.emitter || emitterName;

    let tableHtml = `<h3>${emitterLabel} 的預測結果</h3>`;
    tableHtml += `<table style="${tableStyle}">`;
    ...
    tableHtml += `<tr>
        <td style="${tdStyle}">PLQY</td>
        <td style="${tdStyle}">${formattedPredPLQY}</td>
    </tr>`;

    tableHtml += `<tr>
        <td style="${tdStyle}">EQE</td>
        <td style="${tdStyle}">${formattedPredEQE}</td>
    </tr>`;
...
function createMaterialPropertiesTable(material) {
    const tableStyle = 'border: 1px solid black; border-collapse: collapse; width: 50%;';
    const thStyle = 'border: 1px solid black; padding: 8px; background-color: #f2f2f2; text-align: center;';
    const tdStyle = 'border: 1px solid black; padding: 8px; text-align: center;';

    // PLQY
    const expPLQY = material.plqy;
    const predPLQY = material["plqy.1"];

    let formattedPredPLQY = "N/A";
    let errorPLQY = "N/A";
    if (typeof predPLQY === "number") {
        formattedPredPLQY = predPLQY.toPrecision(3);
        errorPLQY = calculateError(expPLQY, predPLQY) + "%";
    }

    // EQE
    const expEQE = material.EQE;
    const predEQE = material["EQE.1"];

    let formattedPredEQE = "N/A";
    let errorEQE = "N/A";
    if (typeof predEQE === "number") {
        formattedPredEQE = predEQE.toPrecision(3);
        errorEQE = calculateError(expEQE, predEQE) + "%";
    }

    // 建立表格
    let tableHtml = `<h3>${material.emitter} 的材料性質</h3>`;
    tableHtml += `<table style="${tableStyle}">`;
    ...
    tableHtml += `<tr>
        <td style="${tdStyle}">PLQY</td>
        <td style="${tdStyle}">${expPLQY ?? "N/A"}</td>
        <td style="${tdStyle}">${formattedPredPLQY}</td>
        <td style="${tdStyle}">${errorPLQY}</td>
    </tr>`;

    // EQE 列
    tableHtml += `<tr>
        <td style="${tdStyle}">EQE</td>
        <td style="${tdStyle}">${expEQE ?? "N/A"}</td>
        <td style="${tdStyle}">${formattedPredEQE}</td>
        <td style="${tdStyle}">${errorEQE}</td>
    </tr>`;
...
// 計算誤差（百分比）
function calculateError(expValue, predictedValue) {
    if (typeof expValue !== "number" || typeof predictedValue !== "number" || expValue === 0) {
        return "N/A";
    }
    return ((Math.abs(predictedValue - expValue) / expValue) * 100).toFixed(2);
}
